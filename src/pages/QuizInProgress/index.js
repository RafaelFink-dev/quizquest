...
export default function QuizInProgress({ conteudo }) {
  ...
  useEffect(() => {
    ...
    async function loadQuiz() {
      ...
      //setTimeLeft(quizDoc.data().tempoResposta)
      ...
      if (quizDoc.exists()) {
        ...
        setIncorrectAnswers(new Array(loadedQuestions.length).fill(false));
      } else {
  ...
  const handleAnswerSelect = (index) => {
    ...
    if (index === questions[currentQuestionIndex].resposta_correta) {
      ...
      setIncorrectAnswers(prevIncorrect => {
        ...
        updatedIncorrect[currentQuestionIndex] = true;
          ...
          <div className='container-profile'>
            <div className="quiz-container-result">
              <div className='title-finish'>
                <h2>Parabéns você concluíu o quiz!</h2>
              </div>
                ...
                <ul>
                  {questions.map((question, index) => (
                    incorrectAnswers[index] && (
                      <li key={index}>{question.pergunta} - Sua resposta: {questions[index].respostas[selectedAnswer]}, Resposta correta: {question.respostas[question.resposta_correta]}</li>
                    )
                  ))}
                </ul>
              </div>
